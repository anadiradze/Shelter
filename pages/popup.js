...
function createPopupEl({name,image,type,breed,description,age,inoculations,diseases,parasites}) {
  ...
  .then((data) => {
    ...
    for (let i = 0; i < friendElements.length; i++) {
      friendElements[i].addEventListener("click", (e) => {
        ...
        createPopupEl({name,image,type,breed,description,age,inoculations,diseases,parasites})
        inDotThen()
      });
    }
  });
